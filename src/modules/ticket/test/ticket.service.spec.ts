...
import { Ticket, TicketDepartment, TicketStatus } from "../../../models/ticket.model"
...
describe("Ticket Service", () => {
    ...
    beforeEach(async () => {
        ...
        user = em.create(User, { role, username: "fake-username", email: "fake-email", password: "fake-pass", phone: "fake-phone" }, { persist: true })
        await em.flush();
    })
    ...
    describe("Get Ticket By Id", () => {
        ...
        it("should be return ticket By Id", async () => {
            const ticket = await service.create({ body: "fake ticket 1", department: TicketDepartment.sales, title: "test title" }, user.id);
    ...
    describe("remove Tickets", () => {
        ...
        it("should be throw NotFound if the ticket is not found", () => {
            const remPromise = service.remove(122);
        ...
        it("should be remove ticket", async () => {
            const result = await service.remove(ticket.id);
            expect(result).toBeTruthy();
            expect(result.id).toStrictEqual(ticket.id);
        })
    })
    describe("update Tickets", () => {
        ...
        it("should be throw NotFound if the ticket is not found", () => {
            const remPromise = service.update(updatedPayload, 122);
            expect(remPromise).rejects.toThrow(NotFoundException)
            expect(remPromise).rejects.toThrow(ErrorMessages.TICKET_NOT_FOUND)
        })

        it("should be remove ticket", async () => {
            const result = await service.update(updatedPayload, ticket.id);
            expect(result).toBeTruthy();
            expect(result.id).toStrictEqual(ticket.id);
